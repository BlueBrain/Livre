...
__device__ float dequantize( const uint8_t& value, const float& scale )
{
    uint8_t sign = value & 0x80;
    uint8_t abs_value = value & 0x7f;
    float dequantized_value = exp2f(float(abs_value) * scale) - 1.0f;
    dequantized_value *= ((sign==0)? -1.0f : 1.0f);
    return dequantized_value;
}


__global__ void cuDecompTensor_quantized_1(
                                   float*   dst,
                             const uint8_t* core,
                             const float*   u1,
                                   int      coreD,
