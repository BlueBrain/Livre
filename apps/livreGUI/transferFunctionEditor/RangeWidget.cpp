    ...
    connect( this, &RangeWidget::rangeXChanged, [this]( qreal value )
            {
                ...
                _ui->minDataRange->setMinimum( std::min( _ui->minDataRange->minimum(), double( value )));
                _ui->minDataRange->setValue( value );
            });

    auto leftRange = _ui->background->rootObject()->findChild<QObject*>("leftrange");
    connect( leftRange, SIGNAL(viewposChanged(qreal)), this, SLOT(setRangeX(qreal)));


    connect( _ui->maxDataRange,
             static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged),
             [this] ( const double value )
             {
    ...
    connect( this, &RangeWidget::rangeYChanged, [this](qreal value)
            {
                if( _histogram.isEmpty( ))
                    return;
                value = _convertFromNormalizedRange( value );
                _ui->maxDataRange->setMaximum( std::max( _ui->maxDataRange->maximum(), double( value )));
                _ui->maxDataRange->setValue( value );
            });

    auto rightRange = _ui->background->rootObject()->findChild<QObject*>("rightrange");
    connect( rightRange, SIGNAL(viewposChanged(qreal)), this, SLOT(setRangeY(qreal)));
}
                                ...
                                const bool isLogScale )
{
    ...
    _ui->background->rootObject()->setProperty( "histogram",
                                                sampleHistogram( _histogram, _isLogScale ));
}
