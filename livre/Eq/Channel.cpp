...
public:
    ...
    void requestData()
    {
        ...
        const uint32_t minDepth = pipe->getFrameData()->getVRParameters()->minDepth;
        const uint32_t maxDepth = pipe->getFrameData()->getVRParameters()->maxDepth;
        const float screenSpaceError = pipe->getFrameData()->getVRParameters()->screenSpaceError;
        const float worldSpacePerVoxel =
                node->getDashTree()->getDataSource()->getVolumeInformation().worldSpacePerVoxel;
        const uint32_t volumeDepth =
                node->getDashTree()->getDataSource()->getVolumeInformation().rootNode.getDepth();
