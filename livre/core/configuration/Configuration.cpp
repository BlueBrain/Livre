...
void Configuration::parseCommandLine(int32_t argc, const char* const argv[])
{
...
ProgramOptionsDescription& Configuration::getGroup_(
    const std::string& groupName)
{
    const auto& i = _descriptions.find(groupName);
    if (i != _descriptions.end())
        return i->second;
    _descriptions.emplace(
        groupName,
        ProgramOptionsDescription{groupName, lunchbox::term::getSize().first});
