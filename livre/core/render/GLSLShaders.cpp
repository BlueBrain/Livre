...
int GLSLShaders::_load(GLSLShaders::Handle& handle, const std::string& shader,
                       const Strings& paths, const Strings& glslCodes LB_UNUSED,
...
int GLSLShaders::loadShaders(const ShaderData& shaderData)
{
    ...
    {
        const int error =
            _load(vertexShader, shaderData.vShader, shaderData.paths,
                  shaderData.glslCodes, GL_VERTEX_SHADER);
        if (!vertexShader)
        {
    ...
    {
        const int error =
            _load(fragmentShader, shaderData.fShader, shaderData.paths,
                  shaderData.glslCodes, GL_FRAGMENT_SHADER);
        if (!fragmentShader)
        {
    ...
    {
        const int error =
            _load(geometryShader, shaderData.gShader, shaderData.paths,
                  shaderData.glslCodes, GL_GEOMETRY_SHADER);
        if (!geometryShader)
        {
...
bool GLSLShaders::checkOpenGLExtension(
    const std::string& extensionName LB_UNUSED)
{
#ifdef GL3_PROTOTYPES
