...
public:
     ...
     */
    LIVRECORE_API LODNode(const NodeId& nodeId, const Vector3ui& blockSize,
     ...
     */
    LIVRECORE_API Vector3ui getAbsolutePosition() const
    {
     ...
     */
    LIVRECORE_API bool operator==(const LODNode& node) const
    {
...
private:
    ...
    Vector3ui _blockSize; //!< Number of voxels in each dimension without
                          //! paddings.
    Boxui _localVoxelBox; //!< Voxelwise position and dimension in reflevel
                          //! resolution in model space.
    Boxf _worldBox; //!< World space box in normalized coordinates [ 0, 1.0 ).
    void _initialize(); //!< Computes the internal values of LODBox (voxel
                        //! extents, block extents, etc ).
    void _computeWorldBox(
        const Vector3ui& levelTotalBlockDimensions); //!< compute world box.
};
