...
namespace
{
void append3dVector(std::vector<float>& container,
                    const livre::Vector3f& vector)
{
...
void append4dVector(std::vector<float>& container,
                    const livre::Vector4f& vector)
{
    container.push_back(vector.x());
    container.push_back(vector.y());
    container.push_back(vector.z());
...
void BoundingAxis::_createTicks(BoundingAxisData& bbAxisData)
{
    ...
    // Order of magnitude. This is used to map the tick size into [1 - 10]
    // range.
    ...
    // edge J
    _createAxisTicks({-x, -y, z}, {x, -y, z}, {0, 1, 0}, {0, 0, -1}, false,
                     bbAxisData);
    _createAxisTicks({-x, -y, z}, {x, -y, z}, {0, 0, -1}, {0, 1, 0}, true,
                     bbAxisData);
    // edge F
    _createAxisTicks({x, -y, z}, {x, y, z}, {-1, 0, 0}, {0, 0, -1}, false,
                     bbAxisData);
    _createAxisTicks({x, -y, z}, {x, y, z}, {0, 0, -1}, {-1, 0, 0}, true,
                     bbAxisData);

    // edge C
    _createAxisTicks({-x, y, z}, {x, y, z}, {0, 0, -1}, {0, -1, 0}, false,
                     bbAxisData);
    _createAxisTicks({-x, y, z}, {x, y, z}, {0, -1, 0}, {0, 0, -1}, true,
                     bbAxisData);

    // edge E
    _createAxisTicks({-x, -y, z}, {-x, y, z}, {0, 0, -1}, {1, 0, -0}, false,
                     bbAxisData);
    _createAxisTicks({-x, -y, z}, {-x, y, z}, {1, 0, 0}, {0, 0, -1}, true,
                     bbAxisData);
    ...
    // edge B
    _createAxisTicks({-x, y, -z}, {-x, y, z}, {0, -1, 0}, {1, 0, 0}, true,
                     bbAxisData);
    _createAxisTicks({-x, y, -z}, {-x, y, z}, {1, 0, 0}, {0, -1, 0}, false,
                     bbAxisData);

    // edge I
    _createAxisTicks({-x, -y, -z}, {-x, -y, z}, {0, 1, 0}, {1, 0, 0}, false,
                     bbAxisData);
    _createAxisTicks({-x, -y, -z}, {-x, -y, z}, {1, 0, 0}, {0, 1, 0}, true,
                     bbAxisData);
